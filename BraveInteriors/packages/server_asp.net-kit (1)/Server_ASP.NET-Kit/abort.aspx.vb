...
Partial Class abort
    ...
    '**************************************************************************************************
    ' Server ASP ABORT Transaction Page
    '**************************************************************************************************
    ' Description
    ' ===========
    '
    ' This page perform builds an Abort POST first for display on screen, for you to view to contents,
    ' then POSTs that data to the Gateway to Abort the deferred transaction you selected, displaying the
    ...
    Protected Sub proceed_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles proceed.Click
        ...
        Dim objUri As New Uri(SystemURL(strConnectTo, "abort"))
        ...
        Else
            ...
            If Left(strstatus, 2) = "OK" Then
                ...
                strSQL = "UPDATE tblOrders SET Status='ABORTED - Successful DEFERRED transaction subsequently ABORTed' WHERE VendorTxCode='" & strVendorTxCode & "'"
                Dim oSQLCommand = New MySqlCommand(strSQL, objConn)
